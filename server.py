...
def Main():
    ...
    for trial in range(0,TRIALS):
        ...
        # divide screen in quadrants, then record for each object, quadrant location
        # later, see if selection of object in quadrant overlaps
...
#             host = raw_input("Please enter host ip number: ")
        ...
        corrident, mark_rt, totalperf, selectorder, selobjidx, uncertain, somewhatcertain, selcor_objidx, selwrong_objidx, objects = markall(objects)
        ...
        for un in uncertain:
            if un > 9:
                print "sending uncertain" + str(un) 
                c.send(str(un))        
            else:
                print "sending uncertain" + "0" + str(un) 
                c.send('0' + str(un)) 
        
        c.send("::") 
            

        for so in somewhatcertain:
            if so > 9:
                print "sending somewhat certain " + str(so) 
                c.send(str(so))        
            else:
                print "sending somewhat certain " + "0" + str(so) 
                c.send('0' + str(so)) 
        ...
        while uncertainrecv:
            ...
            if received == "::":
                ...
                uncertainrecv = False
            if received != "::":    
                uncertain_other.append(int(float(received)))

        somewhatcertainrecv = True
        somewhatcertain_other = []
        while uncertainrecv:
            received = c.recv(2)
            print "received " + received 
            if received == "::":
                break
                somewhatcertainrecv = False
            if received != "::":    
                somewhatcertain_other.append(int(float(received)))
    
                
        print uncertain_other
        ...
        for o in obspos_other:
            ...
            if o in uncertain_other:
                objects[o].thickness = 0
                objects[o].colour = LIGHTRED
            if o in somewhatcertain_other:
